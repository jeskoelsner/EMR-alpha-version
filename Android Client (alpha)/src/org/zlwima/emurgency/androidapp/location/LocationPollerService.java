...
import org.zlwima.emurgency.androidapp.RestProcessor;
...
public class LocationPollerService extends Service {
  private static final String LOCK_NAME_STATIC = "org.zlwima.emurgency.androidapp.location.LocationPoller";
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    private LocationListener listener = new LocationListener() {
       ...
       * start a service to update our location on the
       * server via REST, then exit the polling loop so the 
       * thread terminates.
       */
      public void onLocationChanged(Location location) {
        handler.removeCallbacks(onTimeout);

        Base.Log("\t - \t Location Update found. \n\t - \t Starting RESTful location update service...");        
		
		ParcelableLocation lastLocation = new ParcelableLocation();
		
		//location won't be null anyway. safety first... 
		if( location != null ) {
			...
			lastLocation.setTimestamp( location.getTime() );
		} else {
			Base.Log( "getLastKnownLocation() NULL !!!" );
		}
		
		ParcelableUser user = new ParcelableUser();
		user.setEmail( Globals.EMAIL );
		user.setLocation( lastLocation );

		Intent newIntent = new Intent( Intent.ACTION_SYNC, null, mContext, RestProcessor.class );
		newIntent.putExtra( Shared.USER_OBJECT, user );
		newIntent.putExtra( Shared.RECEIVER, locationPollerParameter.getReceiver() );
		newIntent.putExtra( Shared.COMMAND, Commands.UPDATE_LOCATION );
    ...
    protected void onPreExecute() {
		...
		private void requestLocationUpdate() {
			try {
				...
				Base.Log( "::::::::::: requestLocationUdpate() " );
            }
